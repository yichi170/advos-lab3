...
void signal_handler(int signal, siginfo_t *si, void *unused) {
	...
	for (int i = 0; i < elf->ehdr.e_phnum; i++) {
		...
		if (phdr->p_vaddr <= (uint64_t)si->si_addr &&
			...
			assert((uint64_t)mapped_mem == start_addr);
			...
			// need to map data onto the memory
			if (start_addr < phdr->p_vaddr + phdr->p_filesz) {
				uint64_t seek_off = start_addr - phdr->p_vaddr;
				lseek(elf->fd, phdr->p_offset + seek_off, SEEK_SET);
				uint64_t read_size = end_addr - start_addr;
				if (read(elf->fd, (void *)start_addr, read_size) != read_size) {
...
int main(int argc, char **argv, char **envp)
{
	...
	for (int i = 0; i < elf->ehdr.e_phnum; i++) {
		...
		void *mapped_mem = mmap((void *)shifted_vaddr, PAGE_SIZE,
					PROT_READ | PROT_WRITE | PROT_EXEC,
					MAP_PRIVATE | MAP_ANONYMOUS, -1,
					0);
		...
		if (read(elf->fd, mapped_ptr, PAGE_SIZE) != PAGE_SIZE) {
			err_quit("Load segment to memory");
		}
		...
		printf("segment mapped to this region: %#lx - %#lx\n", (uint64_t)mapped_mem, (uint64_t)mapped_mem + PAGE_SIZE);
		printf("==============================\n");
	}
